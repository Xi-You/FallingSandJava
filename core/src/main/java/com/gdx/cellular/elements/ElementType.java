...
public enum ElementType {
    ...
    SLIMEMOLDMOVABLE(SlimeMoldMovable.class, ClassType.MOVABLESOLID) {
        @Override
        public Element createElementByMatrix(int x, int y) {
            return new SlimeMoldMovable(x, y);
        }
    },
    ...
    List<ElementType> ElementTypes = new ArrayList<>();
        for (int i = 0; i < ElementType.values().length; i++) {
            if (ElementType.values()[i].classType.equals(classType)){
                ElementTypes.add(ElementType.values()[i]);
            }
        }
        return ElementTypes;//Arrays.stream(ElementType.values()).filter(elementType -> elementType.classType.equals(classType)).collect(Collectors.toList());
    }
...
if (matrix.isWithinBounds(x, y)) {
Element newElement = new Particle(x, y, vector3, elementType);
            matrix.setElementAtIndex(x, y, newElement);
            return newElement;
            }
            return null;
